...
router.post('/', async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (!finalGroupId) {
        // Handle non-group expense
        const participantIds = Array.from(new Set([paidById, ...splits.map((s: any) => s.userId)]));
        
        // Create a hidden group for this non-group expense
        const newGroup = await tx.group.create({
          data: {
            name: 'Non-group Expense',
            description: 'Autogenerated for non-group split',
            type: 'non-group',
