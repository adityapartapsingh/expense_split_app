    ...
    <div className="min-h-screen bg-bg-primary flex font-sans text-text-main">
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 sm:p-12 overflow-y-auto">
        <div className="w-full max-w-md animate-in">
          ...
          <div className="flex items-center gap-4 mb-6">
            ...
            <span className="text-xs font-bold text-text-tertiary uppercase tracking-wider">Or login with email</span>
            <div className="flex-1 h-px bg-border-subtle"></div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <div>
              <label className="block text-sm font-bold text-text-secondary mb-1">Email, Username, or Phone</label>
              <input type="text" name="login" className="w-full px-4 py-3 rounded-xl bg-bg-secondary border border-border-subtle focus:border-brand-accent focus:ring-1 focus:ring-brand-accent outline-none transition-all" value={formData.login} onChange={handleChange} placeholder="aisha@example.com" required />
            </div>
            ...
            <button type="submit" disabled={isSubmitting || !formData.login} className="mt-2 w-full px-6 py-4 rounded-xl font-bold text-white bg-brand-accent hover:bg-sky-500 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-lg shadow-brand-accent/20">
              {isSubmitting ? 'Signing in...' : 'Sign In'}
            </button>
            
            <div className="text-center mt-4">
              <span className="text-text-muted">Don't have an account? </span>
              <Link href="/register" className="font-bold text-brand-accent hover:underline">Sign up</Link>
            </div>
          </form>
        </div>
      </div>
      ...
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden bg-bg-secondary">
        ...
        <div className="relative z-20 mt-auto p-12 text-white">
          ...
          <h2 className="text-5xl font-black tracking-tight mb-4 drop-shadow-md">Welcome back.</h2>
          <p className="text-xl opacity-90 max-w-md drop-shadow-md">Login to continue managing your shared groups and track your financial anomalies.</p>
        </div>
      </div>
    </div>
  );
